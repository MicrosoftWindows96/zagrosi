...
//!
//! ## Forward-compat with the tenant-isolation layer's RLS
//!
//! [`with_org_context`] sets the `app.current_org_id` GUC for the
//! duration of a transaction. Today the GUC is unused by any RLS
//! policy; the tenant-isolation layer turns it into the source for
//! tenant-isolation predicates. Embedding the call now means the
//! tenant-isolation layer lands without identity-side edits.
...
/// Set the per-transaction `app.current_org_id` GUC.
///
/// Today this is forward-compat scaffolding for the tenant-isolation
/// layer's RLS; no RLS policy reads the GUC yet, so calling this from
/// production code is a documented no-op. Once the tenant-isolation
/// layer lands, every multi-tenant query
/// in the identity surface must run inside a transaction that has
/// called this function; the RLS policies will refuse rows whose
/// `org_id` does not match the GUC.
///
/// Implemented via `set_config('app.current_org_id', $1, true)` so
/// the call composes inside a `query!` invocation without parser
/// quirks (`SET LOCAL` is a top-level statement and cannot be
/// parameterised).
